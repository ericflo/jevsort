...
import jevsort
from jevsort import Dimension, as_dimensions, as_items
...
def test_one_liner_returns_the_callers_objects():
    ...
    r = jevsort.sort(items, "Which greeting is warmer?", judge=longer)
...
def test_dicts_keep_identity_and_fields():
    ...
    r = jevsort.sort(items, "Which is longer?", judge=longer)
    assert r.ids == ["y", "x"] and r.sorted[0] is items[1]
    r2 = jevsort.sort({"a": "x", "b": "xxx", "c": "xx"}, "Which is longer?", judge=longer)
...
def test_builder_reaches_every_option():
    r = (jevsort.sorter().by("Which is longer?", brevity="Which is shorter?").judge(longer)
...
def test_power_keywords_pass_through():
    r = jevsort.sort(["a", "bb", "ccc", "dddd"], "Which is longer?", judge=longer, coupling="bt", both_orders=False, seed=3)
    assert r.config["both_orders"] is False and r.per_dim["longer"].method == "bt"


def test_compare_and_function_judge_forms():
    assert jevsort.compare("long text", "short", "Which is longer?", judge=longer) > 0.99
    assert jevsort.compare("a", "bb", judge=lambda q, a, b: "B" if len(b) > len(a) else "A") < 0.01
    seen = []
    ...
    jevsort.sort(["x", "y"], "Which?", judge=with_context, objective="be kind")
    assert seen[0] == "be kind"


def test_jevsorter_accepts_the_easy_forms_too():
    r = jevsort.JevSorter(longer, "Which is longer?").sort(["a", "ccc", "bb"])
    assert r.sorted == ["ccc", "bb", "a"]


def test_hard_votes_use_bradley_terry_not_saturated_eq7():
    r = jevsort.sort(["a", "bb", "ccc", "dddd"], "Which is longer?", judge=longer)
