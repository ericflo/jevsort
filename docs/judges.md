...
| `--backend` | judge | notes |
|---|---|---|
| `openrouter` *(default)* | **Jev 1.13 via OpenRouter** — model `typesafe/jev-1.13` | Decisions API `POST /api/alpha/decisions` (or `PAIRSORT_JEV_SURFACE=systemone` for `POST /api/v1/systemone`); `OPENROUTER_API_KEY`; $0.042 / M input tokens, output free. Falls back to `llm` with a loud warning if Jev is unreachable (`--no-fallback` to fail instead). |
...
| `jev-wire:URL[#MODEL]` | any `/v1/systemone` server | [`ekzhang/openjev-sglang`](https://github.com/ekzhang/openjev-sglang), `decider.serve`, Decision-1.0 endpoints, `pairsort serve` |
...
| model | size | how pairsort runs it |
|---|---|---|
...
| [pngwn/system-one-qwen3.5-4b-scorer-v2b](https://huggingface.co/pngwn/system-one-qwen3.5-4b-scorer-v2b) | 4B LoRA | `--backend hf:…` + `pairsort calibrate` for temperature scaling |
...
The in-process adapters wrap each project's own published Python API (lazy imports, so pairsort itself only needs
`numpy` and `httpx`); they are thin and have not all been exercised against every checkpoint — issues and PRs welcome.
`pairsort backends` shows what's ready on your machine.

**Serve shim.** `pairsort serve --backend llm` exposes *any* backend as a TypeSafe-compatible `POST /v1/systemone`
...
Jev ingests the state once and answers every question against it, so pairsort puts **all items in one shared state**
...
> until then pairsort falls back to the generic LLM judge with a warning.
