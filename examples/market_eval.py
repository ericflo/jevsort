...
-------------------------------------------------------------
The realized stock return from the close of **Friday 2026-09-18** to the close of **Monday 2026-09-21**, from Yahoo
Finance daily closes, fetched after Monday's close. These prices did not exist before 2026-09-21.

What the judges see (and what they can't)
---------------------------------------
Only the text of each company's SEC Form 8-K filing that EDGAR **accepted after Friday's 16:00 ET close and before
Monday's 09:30 ET open**: the press-release exhibit (EX-99.x) if present, else the 8-K body, truncated. Those texts were
written before Monday's trading, so they can't mention how the stock moved. Judges never see a price, a return, or
any news written after the open. The question is "which stock did better Monday?" — a genuinely hard forecasting task.
...
DATA = HERE / "data" / "market.json"
RESULT = HERE / "results" / "market_eval.json"
...
QUESTION = ("Based only on these two SEC filings, both made public after Friday's market close and before Monday "
...
def collect(args):
    ...
    filings = []
    for d in (FRI.replace("-", ""), MON.replace("-", "")):
    ...
    # prices: Friday close -> Monday close
    out = []
    for rec in by_cik.values():
        t = rec["ticker"].replace(".", "-")
        try:
            ch = json.loads(_get(f"https://query1.finance.yahoo.com/v8/finance/chart/{t}?period1=1789689600&period2=1790208000&interval=1d",
        ...
        if px.get(FRI) is None or px.get(MON) is None or px[FRI] < 3:
            continue
        rec["close_fri"], rec["close_mon"] = round(px[FRI], 4), round(px[MON], 4)
        rec["return"] = px[MON] / px[FRI] - 1
        out.append(rec)
        time.sleep(0.2)
    out.sort(key=lambda r: r["ticker"])
    DATA.write_text(json.dumps({
        "session": MON, "window_et": [FRI_CLOSE.isoformat(), MON_OPEN.isoformat()],
...
def judge(args):
    ...
    dim = [Dimension("move", QUESTION, GUIDANCE)]
